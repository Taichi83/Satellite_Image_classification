...
class MobileNet(object):
    def __init__(self, x_train_tr, x_train_val, y_train_tr, y_train_val, EPOCHS=20, 
        ...
        with tf.name_scope("inputs") as scope:
            ...
            self.X, self.y = tf.placeholder(tf.float32, 
                                  shape=[None, self.x_train_tr.shape[1], self.x_train_tr.shape[2], 
                                         self.x_train_tr.shape[3]]), tf.placeholder(tf.float32, 
                                                                                    shape=[None, self.y_train_tr.shape[1]])
            # batch size 
        ...
        with slim.arg_scope(mobilenet_v2.training_scope()):
            # Nets
            self.logits, _ = mobilenet_v2.mobilenet(self.features, num_classes=self.y_train_tr.shape[1])
            self.v_logits, _ = mobilenet_v2.mobilenet(self.features, num_classes=self.y_train_tr.shape[1], reuse=True)
        ...
        with tf.name_scope("loss_func") as scope:
            ...
            self.v_loss = tf.losses.softmax_cross_entropy(onehot_labels=self.labels, logits=self.v_logits)
        ...
        with tf.name_scope("prediction") as scope:
            ...
            self.correct = tf.equal(tf.cast(self.prediction, tf.int32), tf.cast(tf.argmax(input=self.labels, axis=1), tf.int32))
            self.accuracy = tf.reduce_mean(tf.cast(self.correct, tf.float32))
            
            self.v_prediction = tf.argmax(input=self.v_logits, axis=1)
            self.v_correct = tf.equal(tf.cast(self.v_prediction, tf.int32), tf.cast(tf.argmax(input=self.labels, axis=1), tf.int32))
    ...
    def train(self, SAVE_DIR = '../tmp/models/slim_mobilenet', TB_DIR = '../tensorboard/slim_mobilenet', 
    ...
    def test(self, x_test, y_test, BATCH_SIZE_TS=None):
        ...
        pred = self.sess.run([self.v_prediction])
        return pred[0]
...
# save bar-plot
barplot_classes(labels_oh, cl_nms, save_path="../tmp/figures/slim_mobilenet", filename='class_dist.png')

# visualize sample images
sample_images(images, labels_oh, cl_nms, save_path="../tmp/figures/slim_mobilenet", filename='sample_images.png')
...
# Execute training and validation
model = MobileNet(x_train_tr, x_train_val, y_train_tr, y_train_val, EPOCHS=EPOCHS)
...
eval_labels(y_test_le, pred_test, save_path="../tmp/calc_log/slim_mobilenet", filename='confusion_matrix.csv')